...
sourceFile = fullfile(projectRoot, 'results', 'vx_formal_validation', ...
    'v3', 'runtime', 'VX_ND_formal_raw.mat');
outputBase = fullfile(fileparts(thisFile), 'VX_FIG01_normal_dynamic_estimation');

S = load(sourceFile, 'R');
assert(isfield(S, 'R') && isscalar(S.R), ...
    'VX:V3:Figure01:MissingResult', 'Formal source must contain scalar R.');
R = S.R;
assert(isfield(R, 'metadata') && isfield(R.metadata, 'formalRuntime') ...
    && isequal(R.metadata.formalRuntime, true), ...
    'VX:V3:Figure01:NotFormal', 'Formal runtime evidence is required.');
assert(strcmp(string(R.metadata.caseId), "VX-ND"), ...
    'VX:V3:Figure01:WrongCase', 'VX-FIG-01 requires VX-ND.');
...
assert(size(y,2) == 38 && numel(t) == size(y,1), ...
    'VX:V3:Figure01:Interface', 'Expected aligned N-by-38 estY.');
assert(numel(vxTrue) == numel(t) && numel(axInput) == numel(t), ...
    'VX:V3:Figure01:Alignment', 'Required signals are not aligned.');
...
ax1 = nexttile(tl,1);
plot(ax1,t,axInput,'-','Color',C.black,'LineWidth',2.6);
ylabel(ax1,'A_x / (m·s^{-2})');
title(ax1,'(a) 纵向动态输入','FontWeight','normal');
vx_apply_frozen_vy_axes(ax1,C);
legend(ax1,{'A_x'},'Location','northoutside','Orientation','horizontal','Box','off');
...
hold(ax2,'off');
xlabel(ax2,'时间 / s'); ylabel(ax2,'纵向速度 / (m·s^{-1})');
