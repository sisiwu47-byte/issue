function [simIn,cfg] = configure_vx_cs40_drive_calibration_v4b(candidateId)
%CONFIGURE_VX_CS40_DRIVE_CALIBRATION_V4B Configure one physical-only candidate.
% Creates a generated model copy under results; never edits model/vx.slx.

candidateId=upper(string(candidateId));
switch candidateId
    case "A1", speedTime=[0;3;6.0;9;11.5;16];
    case "A2", speedTime=[0;3;5.5;9;11.5;16];
    case "A3", speedTime=[0;3;5.0;9;11.5;16];
    otherwise, error('VX:V4B:Candidate','Candidate must be A1, A2, or A3.');
end
...
n=fieldnames(source);
for k=1:numel(n)
    assert(strcmp(sha256_file(source.(n{k})),frozen.(n{k})), ...
        'VX:V4B:FrozenSourceHash','Frozen %s hash changed.',n{k});
end

stageRoot=fullfile(root,'results','vx_formal_validation','v4b_cs40_drive_calibration');
controlDir=fullfile(root,'results','vx_formal_validation','v4_cs40','carsim_control_CS40');
...
assert(strcmp(sha256_file(runAll),manifest.runAllSha256)&& ...
    strcmp(sha256_file(simfile),manifest.simfileSha256),'VX:V4B:ControlHash');
...
assert(strcmp(manifest.initialSpeedToken,'SV_VXS')&&strcmp(manifest.initialSpeedUnit,'km/h'), ...
    'VX:V4B:InitialSpeedMetadata');
assert(abs(activeInitialSpeed-40)<=0.1,'VX:V4B:PreInitialSpeedGate', ...
    'CURRENT_CARSIM_RUN_NOT_SAVED_AS_40_KMH');
assert(abs(activeMu-0.30)<=1e-12,'VX:V4B:PreMuGate','MU_ROAD_CONSTANT is not 0.30.');
assert(abs(activeStopTime-16)<=1e-12,'VX:V4B:PreStopTimeGate','TSTOP is not 16 s.');

modelDir=fullfile(stageRoot,'configured',char(candidateId),'model');
if ~isfolder(modelDir),mkdir(modelDir);end
modelName=['vx_cs40_v4b_' lower(char(candidateId))];
...
cfg=struct('stage','VX-V4B-CS40-DRIVE-CALIBRATION','candidateId',char(candidateId), ...
    'physicalOnlyCalibration',true,'simulationInvoked',false,'referenceUnit','km/h', ...
    'referenceWasDividedBy3p6',false,'speedTime_s',speedTime,'speed_kmh',speedKmh, ...
    ...
    'preRunMuGatePass',abs(activeMu-0.30)<=1e-12,'sourceFiles',source, ...
    'frozenSourceHashes',frozen,'generatedModel',modelFile, ...
    'runtimeWorkingDirectory',controlDir,'runAll',runAll,'simfile',simfile, ...
    'controlManifestFile',manifestFile,'controlManifest',manifest, ...
    'controlHashes',struct('runAll',sha256_file(runAll),'simfile',sha256_file(simfile), ...
    'manifest',sha256_file(manifestFile),'sourceCpar',manifest.sourceCparSha256), ...
    'generatedModelHash',sha256_file(modelFile));
cfgDir=fullfile(stageRoot,'configured',char(candidateId));
save(fullfile(cfgDir,'case_configuration.mat'),'cfg');
write_json(fullfile(cfgDir,'case_configuration.json'),cfg);
...
line=get_param(ports.Outport(1),'Line');assert(line~=-1,'VX:V4B:SteeringRoute');
...
for k=1:4
    b=[modelName '/' blocks{k}];assert(getSimulinkBlockHandle(b)~=-1&&strcmp(get_param(b,'GotoTag'),tags{k}),'VX:V4B:DuplicateTagAudit');
    used=allFrom(cellfun(@(x)strcmp(get_param(x,'GotoTag'),tags{k}),allFrom));
    assert(isempty(used),'VX:V4B:DuplicateTagUsed');set_param(b,'Commented','on');
end
end
function value=token_number(text,name)
token=regexp(text,['(?m)^' regexptranslate('escape',name) '[ \t]+([-+0-9.eE]+)[ \t]*(?=\r?$)'],'tokens','once');
assert(~isempty(token),'VX:V4B:MissingToken','Missing token %s.',name);value=str2double(token{1});
assert(isfinite(value),'VX:V4B:InvalidToken','Invalid numeric token %s.',name);
end
function write_json(file,value)
fid=fopen(file,'wt');assert(fid>=0,'VX:V4B:JsonWrite');c=onCleanup(@()fclose(fid));
