function report = run_vy_fixed_fusion_v2_5i2_H02_holdout()
%RUN_VY_FIXED_FUSION_V2_5I2_H02_HOLDOUT Dedicated one-shot FWHOLD_H02 runner.
...
assert(isfile(runnerFile),'V25I2:RunnerSelfPathMissing','Runner file not found: %s',runnerFile);
runnerHash=sha256(runnerFile);
assert(isfile(phaseFile),'V25I2:Bootstrap','Durable bootstrap markers are missing.');
assert(~isfile(commitFile),'V25I2:Authorization','Authorization commit already exists; no runtime is permitted.');
append_phase(phaseFile,'RUNNER_ENTERED','FWHOLD_H02',runnerHash);
...
assert(width(pre)==32&&all(ismember(required,pre.Properties.VariableNames))&&~any(startsWith(string(pre.Properties.VariableNames),'Var')), ...
    'V25I2:Preregistration','Frozen preregistration schema mismatch.');
h02=pre(pre.execution_order==2,:); assert(height(h02)==1&&h02.run_id=="FWHOLD_H02",'V25I2:Identity','Execution order 2 is not exact FWHOLD_H02.');
assert(h02.role=="HOLDOUT_VALIDATION"&&double(h02.original_registry_row)==8&&h02.original_status=="PLANNED_NOT_RUN"&& ...
    h02.runtime_authorization=="UNCONSUMED"&&double(h02.runtime_count)==0&&h02.data_viewed=="FALSE", ...
    'V25I2:Untouched','FWHOLD_H02 is not untouched.');
assert(double(h02.steering_amplitude)==0.035&&double(h02.steering_frequency)==0.35&&double(h02.duration)==16&& ...
    double(h02.estimator_rate)==100&&h02.waveform=="SINE_FRONT_EQUAL_REAR_ZERO"&& ...
    h02.front_steering_policy=="FL_FR_SAME_PHASE"&&h02.rear_steering_policy=="RL_RR_ZERO", ...
    'V25I2:Condition','Frozen FWHOLD_H02 condition mismatch.');
resultFile=fullfile(root,strrep(char(h02.formal_result_path),'/',filesep));
assert(~isfile(resultFile),'V25I2:ResultExists','Formal FWHOLD_H02 result already exists.');
for q=1:height(pre)
    if pre.execution_order(q)~=2
        p=fullfile(root,strrep(char(pre.formal_result_path(q)),'/',filesep));
        assert(~isfile(p)&&double(pre.runtime_count(q))==0&&pre.data_viewed(q)=="FALSE",'V25I2:Isolation','Another holdout is not untouched.');
    end
end
append_phase(phaseFile,'PREREGISTRY_PARSED','FWHOLD_H02',char(h02.formal_result_path));
...
assert(height(w)==1&&w.weight_set_id=="V25_FIXED_WEIGHT_ALPHA_V1"&& ...
    ...
    'V25I2:Weights','Frozen runtime weights mismatch.');
assert(strcmpi(char(w.formal_target_sha256),targetExpected)&&strcmpi(char(w.fusion_core_sha256),coreExpected)&& ...
    strcmpi(char(w.fusion_wrapper_sha256),wrapperExpected),'V25I2:Weights','Weight lineage mismatch.');
assert(strcmpi(sha256(target),targetExpected)&&strcmpi(char(h02.target_sha256),targetExpected), ...
    'V25I2:Target','Formal target hash mismatch.');
assert(h02.target_path=="model/vx_vy_fixed_fusion_v2_5.slx",'V25I2:Target','Frozen target path mismatch.');

amp=double(h02.steering_amplitude); freq=double(h02.steering_frequency); duration=double(h02.duration); rate=double(h02.estimator_rate);
report=struct('stage','V2.5-I2','runId','FWHOLD_H02','role','PARTIAL_DIAGNOSTIC_HOLDOUT_EVIDENCE', ...
    'preregistration',table2struct(h02),'runnerSHA256',runnerHash,'targetSHA256',targetExpected, ...
...
try
    ...
    assert(isfile(simFile)&&isfile(solverDll)&&isfile(solverMex)&&isfile(solverLib),'V25I2:CarSim','Required D: runtime files are missing.');
    ...
    assert(strcmpi(report.carSim.progDir,'D:\carsim\CarSim2021.0_Prog\')&& ...
        ...
        'V25I2:CarSim','CarSim path policy mismatch.');
    ...
    assert(strcmpi(pwd,md)&&strcmpi(report.carSim.activeSimfile,simFile)&&strcmpi(report.carSim.mexActual,solverMex),'V25I2:Environment','Project cwd/simfile/MEX mismatch.');
    ...
    assert(report.static.pass&&isequal(report.static.weights,alpha),'V25I2:Static','Frozen target parameters mismatch.');
    ...
    assert(all(cellfun(@(n)numel(find_system(modelName,'SearchDepth',1,'BlockType','ToWorkspace','VariableName',n))==1,logs)), ...
        'V25I2:Logging','Required runtime logger missing.');report.logVariables=logs;
    ...
    report.runCard=struct('runId','FWHOLD_H02','amplitudeRad',amp,'frequencyHz',freq,'durationS',duration,'rateHz',rate, ...
        'waveform',char(h02.waveform),'frontPolicy',char(h02.front_steering_policy),'rearPolicy',char(h02.rear_steering_policy), ...
        'speedScope',char(h02.speed_scope),'truthAlignment',char(h02.truth_alignment_rule),'evaluationWindow',char(h02.evaluation_window_rule));
    ...
    append_phase(phaseFile,'PRE_SIM_GATES_PASS','FWHOLD_H02',targetExpected);
    write_commit(commitFile,'FWHOLD_H02',targetExpected,runnerHash,'V25_FIXED_WEIGHT_ALPHA_V1',alpha,resultFile);
    c=readtable(commitFile,'Delimiter',',','VariableNamingRule','preserve','TextType','string');
    assert(height(c)==1&&c.run_id=="FWHOLD_H02"&&c.authorization_state=="CONSUMED"&& ...
        strcmpi(c.target_sha256,targetExpected)&&double(c.alpha_D)==alpha(1)&&double(c.alpha_K)==alpha(2)&&double(c.alpha_F)==alpha(3), ...
        'V25I2:CommitReadback','Authorization commit read-back failed.');
    report.authorizationCommitSHA256=sha256(commitFile);report.runtimeAuthorization='CONSUMED';
    append_phase(phaseFile,'SIM_AUTHORIZATION_COMMITTED','FWHOLD_H02',report.authorizationCommitSHA256);
    ...
    append_phase(phaseFile,'SIM_RETURNED','FWHOLD_H02','unique sim returned');
    ...
    assert(report.carSimRun,'V25I2:CarSim','CarSim completion evidence missing.');
...
if report.simulationCompleted&&report.carSimRun&&report.frozenHashesUnchanged
    assert(~isfile(resultFile),'V25I2:ResultExists','Formal result appeared before save.');save(resultFile,'report','-v7.3');
    info=dir(resultFile);append_phase(phaseFile,'FORMAL_MAT_SAVED','FWHOLD_H02',sprintf('%s|%d',sha256(resultFile),info.bytes));
end
cleanup(modelName,oldPwd,oldPath);clear cleanupObj
if ~report.simulationCompleted,error('V25I2:RuntimeFailed','Unique runtime failed after durable authorization commit: %s',report.runtimeError.message);end
end
...
assert(~isfile(file),'V25I2:CommitExists','Commit marker already exists.');fid=fopen(file,'w');assert(fid>=0,'V25I2:CommitWrite','Cannot create commit marker.');
...
fprintf(fid,'%s,%s,V2.5-I2,%s,%s,%s,%.17g,%.17g,%.17g,%s,PASS,CONSUMED,UNIQUE_SIM_CALL\n',runId,datestr(now,30),targetHash,runnerHash,weightSet,alpha(1),alpha(2),alpha(3),strrep(resultFile,'\','/'));
assert(fclose(fid)==0,'V25I2:CommitClose','Commit marker close failed.');d=dir(file);assert(isfile(file)&&d.bytes>0,'V25I2:CommitDurability','Commit marker did not persist.');
end
function append_phase(file,phase,runId,detail)
isNew=~isfile(file);fid=fopen(file,'a');assert(fid>=0,'V25I2:PhaseWrite','Cannot append phase marker.');if isNew,fprintf(fid,'timestamp,phase,run_id,detail\n');end
fprintf(fid,'%s,%s,%s,"%s"\n',datestr(now,30),phase,runId,strrep(char(detail),'"','""'));assert(fclose(fid)==0,'V25I2:PhaseClose','Phase marker close failed.');d=dir(file);assert(d.bytes>0,'V25I2:PhaseDurability','Phase marker did not persist.');
end
...
if pos>numel(s),error('V25I2:StaticParameterParse','Empty static parameter expression.');end
[v,pos]=parse_expr_sum(s,pos);pos=skip_expr_ws(s,pos);
if pos<=numel(s),error('V25I2:StaticParameterParse','Unexpected character at position %d.',pos);end
...
while true
    ...
    if op=='*'
        ...
        if rhs==0,error('V25I2:StaticParameterParse','Division by zero is not allowed.');end
        v=v/rhs;
    end
    require_finite_expr(v);
end
...
pos=skip_expr_ws(s,pos);
if pos>numel(s),error('V25I2:StaticParameterParse','Expected a factor.');end
if any(s(pos)=='+-')
...
pos=skip_expr_ws(s,pos);
if pos>numel(s),error('V25I2:StaticParameterParse','Expected a primary expression.');end
if s(pos)=='('
    [v,pos]=parse_expr_sum(s,pos+1);pos=skip_expr_ws(s,pos);
    if pos>numel(s)||s(pos)~=')',error('V25I2:StaticParameterParse','Missing closing parenthesis.');end
...
if isstrprop(s(pos),'alpha')
    ...
    if ~strcmp(identifier,'pi'),error('V25I2:StaticParameterParse','Unknown identifier: %s',identifier);end
    v=pi;return
end
token=regexp(s(pos:end),'^(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?','match','once');
if isempty(token),error('V25I2:StaticParameterParse','Expected a finite numeric literal or pi at position %d.',pos);end
...
if ~isfinite(v),error('V25I2:StaticParameterParse','Static parameter expression must remain finite.');end
end
function r=record_timeseries(ts),assert(isa(ts,'timeseries'),'V25I2:LogType','Expected timeseries.');r=struct('time',double(ts.Time(:)),'data',double(ts.Data),'sampleCount',numel(ts.Time),'dataSize',size(ts.Data));end
function v=macro_value(txt,key),t=regexp(txt,['(?m)^' key '\s+([^\r\n]+)'],'tokens','once');assert(~isempty(t),'V25I2:CarSim','Missing %s.',key);v=strtrim(t{1});end
