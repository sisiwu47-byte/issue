function report = run_vy_parallel_dk_v2_3c_preflight()
%RUN_VY_PARALLEL_DK_V2_3C_PREFLIGHT One authorized 0.20-s parallel run.

root = fileparts(fileparts(mfilename('fullpath')));
md = fullfile(root,'model');
target = fullfile(md,'vx_vy_parallel_dk_v2_3.slx');
...
report = struct();
report.stage = 'V2.3-C single 0.20-s parallel D/K runtime preflight';
report.stopTimeRequested_s = 0.20;
report.nominal = struct('Vx_mps',20,'steerAmplitude_rad',0.02, ...
    'steerFrequency_Hz',0.4,'rearSteer_rad',[0 0]);
...
assert(hashes_match(report.frozenBefore,expectedHashes), ...
    'Frozen hash mismatch before V2.3-C runtime.');
...
assert(strcmpi(report.targetHashBefore, ...
    ...
    'Formal parallel target hash mismatch before runtime.');
...
assert(isfile(formalFile) && isfile(harnessFile), ...
    'Required V2.3-B/B2 evidence is missing.');
F = load(formalFile,'report'); H = load(harnessFile,'report');
assert(F.report.staticPassed && F.report.staticGateCount==41 && ...
    F.report.staticGatesTrue==41,'Formal 41/41 static evidence is invalid.');
assert(H.report.staticPassed && H.report.staticGateCount==38 && ...
    H.report.staticGatesTrue==38 && H.report.compile.passed && ...
    H.report.compiledGateCount==15 && H.report.compiledGatesTrue==15, ...
    'Estimator-only 38/38 static or 15/15 compiled evidence is invalid.');
report.priorEvidence = struct('formalStatic',[41 41], ...
    'harnessStatic',[38 38],'compiled',[15 15]);
...
try
    ...
    Simulink.fileGenControl('set','CacheFolder', ...
        fullfile(tempdir,'vy_parallel_dk_v2_3c_cache'), ...
        'CodeGenFolder',fullfile(tempdir,'vy_parallel_dk_v2_3c_codegen'), ...
        'createDir',true);
    cd(md);
    ...
    logVariables = {'steer_cmd_rad','steer_to_carsim_deg', ...
        ...
        'dekf_diag_log','parallel_input_log'};
    report.static = static_preflight(model,logVariables);
    assert(report.static.pass,'V2.3-C target/log/routing static preflight failed.');

    w = get_param(model,'ModelWorkspace');
    assignin(w,'test_speed',20);
    assignin(w,'test_steer_amplitude',0.02);
    assignin(w,'test_steer_frequency',0.4);
    assignin(w,'vy_v17_mode_code',20);
    assignin('base','test_speed',20);
    ...
    report.carSimRun = contains(console,'Termination at simulation time') && ...
        strcmpi(report.carSim.solverActual,solverDll) && ...
        ~report.carSim.gRequestConsole;
    assert(report.carSimRun,'Authorized CarSim completion evidence is missing.');
    for k = 1:numel(logVariables)
        name = logVariables{k};
        report.raw.(name) = record_timeseries(out.get(name));
    end
    report.stopTimeActual_s = max([report.raw.dekf_x_log.time(end), ...
        report.raw.kkf_x_log1.time(end)]);
...
if ~report.simulationCompleted
    error('VY_PARALLEL_DK:V23CRuntimeFailed', ...
        'V2.3-C runtime failed; evidence saved to %s',resultFile);
end
assert(report.targetHashUnchanged && report.allFrozenHashesUnchanged, ...
    'A formal/frozen file changed during V2.3-C runtime.');
fprintf(['V23C_RUNTIME_OK|simCalled=%d|completed=%d|carSim=%d|' ...
...
audit = struct(); audit.logVariables = names; audit.logPaths = cell(size(names));
loggingOK = true;
for k = 1:numel(names)
    b = find_system(m,'SearchDepth',1,'BlockType','ToWorkspace', ...
        'VariableName',names{k});
    audit.logPaths{k} = b;
    loggingOK = loggingOK && numel(b)==1;
end
...
for k = 1:6
    audit.sharedSources{k} = source_of([m '/Parallel Physical Input Log Mux'],k);
end
audit.dSteerWheelSources = cell(1,4);
for k = 1:4
    audit.dSteerWheelSources{k} = source_of([m '/Parallel D Steering Mux'],k);
end
audit.kResetSource = source_of([m '/parallel_input_log'],1);
...
audit.steerSource = struct('block',cmd,'amplitude',get_param(cmd,'Amplitude'), ...
    'frequency',get_param(cmd,'Frequency'),'phase',get_param(cmd,'Phase'), ...
    'bias',get_param(cmd,'Bias'),'sampleTime',get_param(cmd,'SampleTime'));
audit.radToDegGain = get_param(gain,'Gain');
audit.manualSwitch = struct('sw',get_param(sw,'sw'), ...
    'currentSetting',get_param(sw,'CurrentSetting'), ...
    'selectedSource',source_of([m '/Mux7'],2));
audit.genuineSteeringDefinitionOK = ...
    strcmp(audit.steerSource.amplitude,'test_steer_amplitude') && ...
    strcmp(audit.steerSource.frequency,'2*pi*test_steer_frequency') && ...
    strcmp(strtrim(audit.radToDegGain),'180/pi') && ...
    strcmp(audit.manualSwitch.sw,'0') && ...
    strcmp(audit.manualSwitch.currentSetting,'0') && ...
    strcmp(audit.manualSwitch.selectedSource,sw);
audit.sharedRoutingOK = strcmp(audit.kAxSource,audit.sharedSources{1}) && ...
    strcmp(audit.kAySource,audit.sharedSources{2}) && ...
    strcmp(audit.kAvzSource,audit.sharedSources{3}) && ...
    strcmp(audit.kVxSource,audit.sharedSources{4}) && ...
    strcmp(audit.dVxSource,audit.sharedSources{4}) && ...
    strcmp(audit.dAySource,audit.sharedSources{2}) && ...
    strcmp(audit.dAvzSource,audit.sharedSources{3}) && ...
    strcmp(audit.dSteeringSource,[m '/Parallel D Steering Mux']) && ...
    strcmp(audit.sharedSources{5},[m '/Parallel D Steering Mux']);
audit.steeringWheelRoutingOK = ...
    strcmp(audit.dSteerWheelSources{1},[m '/G0 Steer Cmd Rad']) && ...
...
audit.pass = audit.loggingOK && audit.sharedRoutingOK && ...
    audit.steeringWheelRoutingOK && audit.schedulersIndependent && ...
    audit.genuineSteeringDefinitionOK;
end

function src = source_of(block,port)
ph = get_param(block,'PortHandles'); lh = get_param(ph.Inport(port),'Line');
assert(lh>0,'Unconnected input: %s/%d',block,port);
src = getfullname(get_param(lh,'SrcBlockHandle'));
end
function src = source_of_trigger(block)
ph = get_param(block,'PortHandles'); assert(~isempty(ph.Trigger));
lh = get_param(ph.Trigger(1),'Line'); assert(lh>0);
src = getfullname(get_param(lh,'SrcBlockHandle'));
end
...
assert(~isempty(token),'CarSim config entry missing: %s',key);
...
for k=1:numel(files)
    records(k)=struct('path',files{k},'sha256',sha256(files{k}));
end
end
