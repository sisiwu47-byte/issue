function report = run_vy_kkf_v2_1c1_nominal()
%RUN_VY_KKF_V2_1C1_NOMINAL Run one authorized 16 s nominal validation case.
...
report = struct();
report.stage = 'V2.1-C1 16 s Nominal Runtime Validation';
report.stopTime = 16;
...
report.qrTuningPerformed = false;
...
try
    ...
    Simulink.fileGenControl('set', ...
        'CacheFolder', fullfile(tempdir, 'vy_kkf_v2_1c1_cache'), ...
        'CodeGenFolder', fullfile(tempdir, 'vy_kkf_v2_1c1_codegen'), ...
        'createDir', true);
    ...
    enable_port_logging([modelName '/K-KF Reset First Call'], ...
        'kkf_reset_trace_c1');
    enable_port_logging([modelName '/Gain38'], 'kkf_vx_true_c1');
    enable_port_logging([modelName '/Gain11'], 'kkf_vy_true_c1');
    ...
    raw.kkf_reset_trace_c1 = timeseries_record( ...
        logged_timeseries(out, 'kkf_reset_trace_c1'));
    raw.vx_true_offline = timeseries_record( ...
        logged_timeseries(out, 'kkf_vx_true_c1'));
    raw.vy_true_offline = timeseries_record( ...
        logged_timeseries(out, 'kkf_vy_true_c1'));
    report.raw = raw;
...
if ~report.simulationCompleted
    fprintf(2, 'V2_1C1_RUNTIME_ERROR|%s|%s\n', ...
        report.runtimeError.identifier, report.runtimeError.message);
    error('VY_KKF:V2_1C1RuntimeFailed', ...
        'V2.1-C1 runtime failed; evidence saved to %s.', resultFile);
end
assert(report.frozenHashesUnchanged, 'A frozen file changed during C1 runtime.');
assert(report.targetMetadataUnchanged, 'Target model metadata changed during C1 runtime.');
assert(report.carSim.simFileUnchanged, 'CarSim simfile changed during C1 runtime.');
fprintf(['V2_1C1_RUNTIME_OK|stop=16|Klogs=%d/%d/%d/%d|' ...
